                                ...
                                bool baseFormatSave )
{
	...
	// DANIEL_BUG_FIX - begin, open reader of vec file of negative examples. Do this using class - positive reader (because negative reader is not suitable for this work).
	if ( !daniel_negReader.create(_negFilename, _negFilename,_cascadeParams.winSize)){
		cout << "Could not read negatives -vec " << _negFilename << "." << endl;
		return false;
	}
	// DANIEL_BUG_FIX - end
    if ( !load( dirName ) ){
...
bool CvCascadeClassifier::updateTrainingSet( double& acceptanceRatio)
{
    ...
    imgReader.restart();											// Restart the positive reader (to take all positive samples). Do not restart the negative!
    int posCount = fillPassedSamples( 0, numPos, true, posConsumed );
    if( !posCount )
        return false;
    cout << "POS count : consumed   " << posCount << " : " << (int)posConsumed << "    --->   ";
	printf("%f%%\n",((posCount*100.0f)/posConsumed));//<< setiosflags(ios::fixed) << setprecision(2) << ((posCount*100.0f)/posConsumed) << resetiosflags(<< "%" << endl;
...
int CvCascadeClassifier::fillPassedSamples( int first, int count, bool isPositive, int64& consumed )
{
    ...
    for( int i = first; i < first + count; i++ ){
        for( ; ; ){
			...
			bool isGetImg = isPositive ? imgReader.getPos( img )   :	daniel_negReader.getPos(img); // DANIEL_BUG_FIX - reading neg examples from vec file
            if( !isGetImg )
                return getcount;
            consumed++;
			...
			if (consumed%10000 == 0)
				printf("Needed %d) Acquired: %d, Consumed %ld \r",count,i-first, consumed); // DANIEL_BUG_FIX - verbose printing
        }
    }
	printf("\n"); // DANIEL_BUG_FIX
    return getcount;

...
bool CvCascadeImageReader::PosReader::get( Mat &_img )
{
    ...
    if( elements_read != 1 ){
		printf("Can not get new sample. The most possible reason is insufficient count of samples in given vec-file.\n");
		return false; // DANIEL_BUG_FIX - reading from vec file (not enough elements. Just exit)
        //CV_Error( CV_StsBadArg, "Can not get new positive sample. The most possible reason is insufficient count of samples in given vec-file.\n");
	}
    elements_read = fread( vec, sizeof( vec[0] ), vecSize, file );
    if( elements_read != (size_t)(vecSize) )
        CV_Error( CV_StsBadArg, "Can not get new sample. Seems that vec-file has incorrect structure.\n");

    if( feof( file ) || last++ >= count ){		
		printf("Can not get new sample. vec-file is over.\n");	
		return false; // DANIEL_BUG_FIX - reading from vec file (not enough elements. Just exit)
